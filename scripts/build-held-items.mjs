...
function main() {
  ...
  // Reborn-holdable items that don't appear in USUM Smogon data — currently the
  // type Gems. Appended so they're always selectable in the inventory picker.
  for (const gem of TYPE_GEMS) {
    const id = toId(gem.gemName);
    if (!byId.has(id)) items.push({ id, name: gem.gemName });
  }
